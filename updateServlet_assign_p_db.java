...
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
@WebServlet("/updateServlet_assign_p_db")
@MultipartConfig(maxFileSize = 16177215)    // upload file's size up to 16MB
public class updateServlet_assign_p_db extends HttpServlet {
    	     ...
    	     // System.out.println("For " + t.size() + "  bbbbb -- "  +  b.size() + " aaa---- " + a.size() + "\n"  ); 
    	      try{
    	           for ( int j = 0; j < b.size();j++)
    	           {
            	         ...
            	         st = "Insert into msr_assign_p(ticket_no,assignee,status,comments,updated_time) values (?,?,?,?,?)";
